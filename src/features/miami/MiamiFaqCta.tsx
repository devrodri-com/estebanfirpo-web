...
import { eyebrowClass, invertedButtonClass, outlineButtonClass, sectionTitleClass } from "./miami-styles";

type MiamiFaqCtaProps = {
  locale: Locale;
  copy: MiamiContent["faq"];
};

export function MiamiFaqCta({ locale, copy }: MiamiFaqCtaProps) {
  return (
    <section
      data-miami-block="faq-cta"
      aria-labelledby="miami-faq-title"
      className="py-16 sm:py-24"
    >
      <div className="max-w-3xl">
        <p className={eyebrowClass}>{copy.eyebrow}</p>
        <h2 id="miami-faq-title" className={sectionTitleClass}>
          {copy.title}
        </h2>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1.18fr_0.82fr] lg:items-start lg:gap-10">
        ...
        <aside className="relative overflow-hidden rounded-2xl bg-[#0A2540] p-6 text-white shadow-[0_18px_55px_rgba(10,37,64,0.14)] sm:p-8 lg:sticky lg:top-24">
          ...
          <p className="mt-4 text-sm leading-6 text-white/72">{copy.ctaCopy}</p>
          ...
          <p className="mx-auto mt-6 max-w-3xl text-center text-xs leading-5 text-white/78">
            {copy.disclaimer}
          </p>
        </aside>
      </div>
